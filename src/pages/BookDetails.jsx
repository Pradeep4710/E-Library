import { Link, useParams } from "react-router-dom"
import { baseUrl } from "../utils/mockData/books";
import { useSelector } from "react-redux";


function BookDetails() {
  const { title } = useParams(); //fetching title from url
  const books = useSelector(state => state.books.items);//fetching books from store
  const book = books.find((b) => b.title.includes(title)); //finding book by title
  const image = book.coverImage ? book.coverImage : `${baseUrl}${book.imageLink}` //there are two different types of book cover image links

  return (
    <div className="flex flex-col md:flex-row space-x-20 my-20 mx-5">
      <figure className="flex justify-center items-center p-1 md:p-5 w-screen md:w-fit bg-orange-400 rounded-lg ring-2 ring-orange-500 shadow-xl ml-0 md:ml-10"><img src={image} alt="book cover image" /></figure>
      <div className="space-y-5 ">
        <h1 className="text-4xl font-serif text-orange-800 mt-3">{book.title}</h1>
        <p className="text-xl">Author : <span className="text-orange-500 font-medium font-serif my-3">{book.author}</span></p>
        <div className="flex flex-col md:flex-row items-center space-x-3">
        <a href={book.link} target="_blank"><button className="flex bg-orange-700 my-2 text-white font-extrabold p-2 px-6 rounded-xl hover:bg-orange-500 transition-colors"><span className="mr-2">Read More</span> <img className="w-5 h-5" alt="svgImg" src="data:image/svg+xml;base64,"/></button></a>
        <Link to="/BrowseBooks"><button className="bg-orange-700 my-2 mx-2 text-white font-extrabold p-2 px-6 rounded-xl hover:bg-orange-500 transition-colors">Back to Browse</button></Link>
        </div>
      </div>
      <div className="flex flex-col space-y-5 text-xl">
        <h6 className="text-orange-500 font-bold font-serif text-2xl">Book Details</h6>
        <p>Language: <span className="text-orange-600 font-semibold">{book.language}</span></p>
        <p>No. of Pages: <span className="text-orange-600 font-semibold">{book.pages}</span></p>
        <p>Country of Origin: <span className="text-orange-600 font-semibold">{book.country}</span></p>
        <p>Published Year: <span className="text-orange-600 font-semibold">{book.year}</span></p>
        
      </div>
      
    </div>
  )
}

export default BookDetails